...
from PyQt6.QtGui import QImage, QPixmap, QDrag, QCursor, QIcon, QPainter, QPen, QColor
...
# --- MODIFICA CHIAVE: Classe QObject separata per emettere segnali ---
...
class TextEditLogger(logging.Handler):
    ...
    # --- MODIFICA: Il segnale è stato spostato in LogEmitter ---
...
# Inizializza il classificatore a cascata di Haar per il rilevamento del volto
face_cascade = None
try:
...
class VideoThread(QThread):
    ...
    def run(self):
        ...
        self.status_signal.emit("✅ Sistema attivo - Mostra le mani o il viso")

        while self._run_flag:
            ret, frame = self.cap.read()
            if ret:
                # Modifica: specchia orizzontalmente il frame per un effetto "a specchio"
                ...
                # Rilevamento mani tramite colore
...
class DraggableTextWidget(QFrame):
    ...
    def toggle_read_text(self):
        """Avvia o ferma la lettura del testo."""
    ...
    def start_reading(self):
        """Avvia la simulazione della lettura del testo."""
        ...
        self.tts_thread = threading.Thread(target=self._simulate_reading, args=(self.text_label.text(),))
        self.tts_thread.start()

    def stop_reading(self):
        """Ferma la simulazione della lettura del testo."""
        if self.is_reading:
    ...
    def _simulate_reading(self, text):
        """Funzione in un thread separato che simula la lettura."""
        logging.info(f"Lettura in corso: {text}")

        words = text.split()
        for word in words:
            if self.stop_signal.is_set():
                break
            time.sleep(0.15) # Simula il tempo di lettura per ogni parola

        # Riporta il pulsante allo stato di riposo al termine della lettura
    ...
    def test_ollama_connection(self):
        ...
        try:
            ...
            if models:
                ...
                # Seleziona il primo modello come richiesto
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # Connessione segnale per la gestione delle risposte Ollama
        self.ollama_thread = None

        # Inizializza le impostazioni, inclusa la connessione a Ollama
        self.init_settings()

    def init_settings(self):
        """Inizializza le impostazioni e tenta di connettersi a Ollama all'avvio."""
        dialog = ConfigurationDialog(self, self.settings)
        # La dialog si connette a Ollama all'inizializzazione, non è necessario visualizzarla
    ...
    def setup_ui(self):
        ...
        # Pannello di Log - Aggiunto qui, inizialmente nascosto
        self.log_widget = QTextEdit()
        self.log_widget.setReadOnly(True)
        self.log_widget.hide()  # Inizialmente nascosto
        ...
        # --- MODIFICA CHIAVE: Inizializza l'emettitore di segnali e l'handler di log in modo corretto ---
    ...
    def setup_top_bar(self, parent_layout):
        ...
        self.status_label = QLabel("✅ Sistema attivo - Mostra le mani o il viso")
    ...
    def setup_central_panels(self, parent_layout):
        ...
        # Pannello Contenuti
        self.contents_panel = ScrollablePanel("📝 Contenuti", "blue")
        panels_layout.addWidget(self.contents_panel, 1)

        # Pannello Area di Lavoro
        self.work_area_panel = ScrollablePanel("🎯 Area di Lavoro", "red")
        ...
        self.ai_panel = ScrollablePanel("🤖 Risultati AI", "green")
    ...
    def send_to_ai(self):
        ...
        # Avvia il thread per la chiamata API
    ...
    def handle_ollama_response(self, response_text):
        ...
        self.add_text(response_text)  # Aggiunge la risposta ai contenuti
        self.ai_results_text.setText(response_text) # Mostra la risposta nel pannello AI
    ...
    def toggle_voice_input(self, checked=False):
        ...
        if not self.is_listening:
            ...
            # Usa il percorso dell'icona corretto
            ...
            # Se la registrazione è già in corso, la ferma
    ...
    def run(self):
        try:
            ...
            payload = {
                "model": self.model,
                "prompt": self.prompt,
                "stream": False # Non usiamo lo streaming per semplicità
            }
...
class VoiceRecognitionThread(QThread):
    ...
    def __init__(self, lang_setting):
        ...
        # Mappa le impostazioni della lingua a codici ISO
        lang_map = {
            'Italiano': 'it-IT',
            'English': 'en-US',
            'Français': 'fr-FR',
